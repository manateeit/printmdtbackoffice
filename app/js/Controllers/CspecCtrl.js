...
MDTCRMCtrls.controller('CspecCtrl', ['$scope','$routeParams','cspecDataSvc','cspecDataShare','$timeout','$http',
                            function ($scope, $routeParams, cspecDataSvc, cspecDataShare, $timeout, $http) {
   if ($routeParams.cspecId == null)
        return;
    else {
     $scope.customerId = $routeParams.customerId;
     $scope.cspecId = $routeParams.cspecId;
     }
        cspecDataSvc.childlookup($scope.customerId,$scope.cspecId, function(result) {
          $timeout (function () {
             $scope.data = result;
          });
        });
